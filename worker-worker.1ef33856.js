...
var $79e099a5c38e77c5$exports = {};

var $kAj3D = parcelRequire("kAj3D");

var $2DkMo = parcelRequire("2DkMo");

var $c4jmL = parcelRequire("c4jmL");
const $79e099a5c38e77c5$var$specialGates = new Set([
    'Subcircuit',
    'Input',
    'Output',
    'Button',
...
class $79e099a5c38e77c5$var$Link {
    constructor(source, target){
        this.source = source;
        this.target = target;
    }
}
class $79e099a5c38e77c5$var$Gate {
    get(name) {
        return this._params[name];
    }
    set(name, value) {
        this._params[name] = value;
        if (this._presentationParams.includes(name)) $79e099a5c38e77c5$var$worker._markPresentationParam(this, name);
    }
    ...
    constructor(id, graph, gateParams, ports, inputSignals, outputSignals){
        this.id = id;
        this.graph = graph;
        this.special = $79e099a5c38e77c5$var$specialGates.has(gateParams.type);
        this.isSubcircuit = gateParams.type == 'Subcircuit';
        this.isOutput = gateParams.type == 'Output';
        const cell = $kAj3D[gateParams.type].prototype;
        ...
        for (const port of ports){
            ...
            if (port.dir == "in") this._params.inputSignals[port.id] = $c4jmL.Vector3vl.fromClonable(inputSignals[port.id]);
            if (port.dir == "out") {
                this._params.outputSignals[port.id] = $c4jmL.Vector3vl.fromClonable(outputSignals[port.id]);
...
class $79e099a5c38e77c5$var$Graph {
    addLink(linkId, source, target) {
        this._links[linkId] = new $79e099a5c38e77c5$var$Link(source, target);
    ...
    addGate(gateId, gateParams, ports, inputSignals, outputSignals) {
        this._gates[gateId] = new $79e099a5c38e77c5$var$Gate(gateId, this, gateParams, ports, inputSignals, outputSignals);
    }
    removeLink(linkId) {
...
class $79e099a5c38e77c5$var$WorkerEngineWorker {
    interval(ms) {
        this._interval = ms;
    }
    ...
    addGraph(graphId) {
        ...
        this._graphs[graphId] = new $79e099a5c38e77c5$var$Graph(graphId);
    }
    ...
    removeLink(graphId, linkId) {
        ...
        const sig = $c4jmL.Vector3vl.xes(targetGate.getPort(link.target.port).bits);
    ...
    changeInput(graphId, gateId, sig) {
        ...
        this._setGateOutputSignals(gate, {
            out: $c4jmL.Vector3vl.fromClonable(sig)
        });
    }
    ...
    manualMemChange(graphId, gateId, addr, data) {
        const gate = this._graphs[graphId].getGate(gateId);
        gate.memdata.set(addr, $c4jmL.Vector3vl.fromClonable(data));
        this._enqueue(gate);
    }
    monitor(graphId, gateId, port, monitorId, { triggerValues: triggerValues , stopOnTrigger: stopOnTrigger , oneShot: oneShot , synchronous: synchronous  }) {
        if (triggerValues != undefined) for (const k of triggerValues.keys())triggerValues[k] = $c4jmL.Vector3vl.fromClonable(triggerValues[k]);
        const gate = this._graphs[graphId].getGate(gateId);
    ...
    alarm(tick, alarmId, { stopOnAlarm: stopOnAlarm , synchronous: synchronous  }) {
    ...
    _postMonitors() {
        for (const [monitorId, sig] of Object.entries(this._monitorChecks)){
            const { triggerValues: triggerValues , stopOnTrigger: stopOnTrigger , oneShot: oneShot , synchronous: synchronous  } = this._monitors[monitorId];
            let triggered = true;
        ...
        if (this._alarmQueue.get(this._tick)) {
            for (const alarmId of this._alarmQueue.get(this._tick)){
                const { stopOnAlarm: stopOnAlarm , synchronous: synchronous  } = this._alarms[alarmId];
                if (synchronous) this._sendUpdates();
                delete this._alarms[alarmId];
                postMessage({
                    type: 'alarmReached',
    ...
    _setGateOutputSignals(gate, newOutputs) {
        for (const [port, sig] of Object.entries(newOutputs))this._setGateOutputSignal(gate, port, sig);
    }
    ...
    _sendUpdates() {
        ...
        if (this._toUpdateParam.size > 0) {
            for (const [gate, params] of this._toUpdateParam)for (const param of params)postMessage({
                type: 'gateSet',
                args: [
                    gate.graph.id,
                    gate.id,
    ...
    constructor(){
        ...
        this._queue = new Map();
        this._pq = new (/*@__PURE__*/$parcel$interopDefault($2DkMo))();
        this._toUpdate = new Map();
        this._toUpdateParam = new Map();
        this._tick = 0;
...
const $79e099a5c38e77c5$var$worker = new $79e099a5c38e77c5$var$WorkerEngineWorker();
self.onmessage = (e)=>{
    const msg = e.data;
    if ('arg' in msg) $79e099a5c38e77c5$var$worker[msg.type](msg.arg);
    else if ('args' in msg) $79e099a5c38e77c5$var$worker[msg.type].apply($79e099a5c38e77c5$var$worker, msg.args);
    else $79e099a5c38e77c5$var$worker[msg.type]();
};
