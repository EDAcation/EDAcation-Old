...
export const execute = async (worker: Worker, file: EditorFileLoaded) => new Promise<string[]>((resolve, reject) => {
    const id = uuidv4();
    ...
    const handleMessage: (event: MessageEvent<{id: string; result: string[]}>) => void = (event) => {
        if (event.data.id === id) {
